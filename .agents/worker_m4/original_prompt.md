## 2026-06-04T17:04:47Z
You are a teamwork_preview_worker.
Your working directory is: d:\archive\.agents\worker_m4
...
1. Update `frontend/src/pages/checkout/components/PaymentStep.jsx`:
   ...
   - Add a prepaid payment option with ID `"online"` (e.g. using an icon like `CreditCard` or `Shield` or standard icons).
...
2. Update `frontend/src/hooks/useCheckout.js` to handle the `"online"` payment method in `handlePlaceOrder`:
   ...
   - Open the Razorpay checkout window with the required configurations:
     ...
     - `handler`: a callback function triggered on payment success. Inside the callback:
       - Send a verification request to `POST /api/payment/razorpay/verify` (or via `paymentService.verifyRazorpayPayment` if defined, or using `api.post`) with the verification details: `razorpay_order_id`, `razorpay_payment_id`, and `razorpay_signature`.
