...
def convert_to_webp(image_path):
    ...
    print(f"Converting local asset: {image_path} -> {webp_path}...")
    try:
        ...
        print(f"Success! Deleted original: {image_path}")
...
def replace_in_file(file_path):
    try:
        ...
        new_content = re.sub(r'([a-zA-Z0-9_\-\/]+)\.(png|jpg|jpeg)', r'\1.webp', content)
...
def main():
    ...
    # 2. Convert frontend/src/ folder images
    ...
    # 3. Replace references in .jsx, .js, .css, .py files
    print("Replacing image references in code...")
    for root, dirs, files in os.walk(frontend_src_dir):
        for file in files:
    ...
    for root, dirs, files in os.walk(backend_dir):
        for file in files:
            if file.endswith('.py') and file not in ('convert_existing_to_webp.py', 'convert_local_assets_to_webp.py'):
                replace_in_file(os.path.join(root, file))
