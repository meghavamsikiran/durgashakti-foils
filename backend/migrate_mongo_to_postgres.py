...
async def ensure_schema(conn):
    ...
    statements = [
        ...
        "CREATE TABLE IF NOT EXISTS products (id UUID PRIMARY KEY, name VARCHAR(255) NOT NULL, description TEXT, size VARCHAR(50), thickness VARCHAR(50), price NUMERIC(12,2) NOT NULL, discount_price NUMERIC(12,2), badge VARCHAR(100), image_url TEXT, features JSONB DEFAULT '[]' NOT NULL, in_stock BOOLEAN DEFAULT TRUE NOT NULL, stock_quantity INTEGER DEFAULT 0 NOT NULL, units_sold INTEGER DEFAULT 0 NOT NULL, low_stock_threshold INTEGER DEFAULT 20 NOT NULL, category VARCHAR(100), batch_no VARCHAR(100), width VARCHAR(50) DEFAULT '295mm', base_name VARCHAR(255), variant_sku VARCHAR(100), created_by UUID, created_at TIMESTAMPTZ DEFAULT now() NOT NULL, updated_at TIMESTAMPTZ)",
