      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          try {
            ...
            console.info(`Fetching address for lat: ${latitude}, lon: ${longitude}`);
            ...
            toast.error('Failed to resolve address from GPS. Please enter manually.');
            resolve(null);
          } finally {
            setLoading(false);
          }
        ...
        (error) => {
          ...
          if (error.code === 1) {
            toast.error('Location permission denied. Please allow location access in your browser address bar.');
          } else if (error.code === 2) {
            toast.error('Position unavailable. Please ensure location services are enabled on your device.');
          } else if (error.code === 3) {
            toast.error('Location request timed out. Please try clicking "Use My Current Location" again.');
          } else {
