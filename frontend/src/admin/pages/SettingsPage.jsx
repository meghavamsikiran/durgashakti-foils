...
const SettingsPage = () => {
  ...
  const saveWhatsAppSettings = async () => {
    try {
      ...
      toast.success('WhatsApp AI Feedback Bot settings saved!');
    } catch (error) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="whitespace-nowrap animate-marquee flex items-center">
                           ...
                           {(!bannerUseFavicon || showFaviconSpacer1) && (
                              <span className="mx-4 text-xs font-black text-primary flex items-center gap-2">
                                 {bannerUseFavicon ? <img src="/favicon.webp" className="w-5 h-5 object-contain opacity-80" alt="Favicon" /> : <Sparkles className="w-4 h-4 text-amber-400 animate-pulse" />}
                              </span>
                           )}
          ...
          {/* WhatsApp AI Feedback Bot Settings */}
          {(me?.role === 'SUPER_ADMIN' || me?.permissions?.manage_settings) && (
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 relative overflow-hidden mt-8">
              <div className="absolute top-0 right-0 p-8 opacity-[0.03] rotate-12 pointer-events-none">
                <MessageSquare className="w-32 h-32 text-emerald-500/20" />
              </div>

              <h2 className="text-lg font-black text-slate-900 uppercase tracking-tighter mb-2 flex items-center gap-2">
                <MessageSquare className="w-5 h-5 text-emerald-600" />
                WhatsApp AI Post-Delivery Feedback Bot
              </h2>
              <p className="text-xs text-slate-500 mb-6 font-medium">
                Automatically trigger post-delivery feedback & damage reporting messages on WhatsApp when an order is marked DELIVERED.
              </p>

              <div className="space-y-6">
                <div className="p-5 rounded-2xl bg-emerald-50/50 border border-emerald-200/60 flex items-center justify-between gap-4">
                  <div>
                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">Automated WhatsApp Triggers</h3>
                    <p className="text-xs text-slate-500 mt-1">Send automatic feedback requests within 1 minute of marking order Delivered.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setWhatsappBotEnabled(!whatsappBotEnabled)}
                    className={`w-14 h-8 flex items-center rounded-full p-1 shrink-0 cursor-pointer transition-all duration-300 shadow-inner ${whatsappBotEnabled ? 'bg-emerald-500' : 'bg-slate-300'}`}
                  >
                    <span className={`bg-white w-6 h-6 rounded-full shadow-md transform transition-all duration-300 ${whatsappBotEnabled ? 'translate-x-6' : 'translate-x-0'}`} />
                  </button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">WhatsApp Business Phone Number</label>
                    <input
                      type="text"
                      placeholder="e.g. 919999999999"
                      value={whatsappBusinessNumber}
                      onChange={e => setWhatsappBusinessNumber(e.target.value)}
                      className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-emerald-500/20 outline-none"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">Include country code (e.g. 91 for India)</p>
                  </div>

                  <div>
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">WhatsApp Cloud API Phone Number ID (Optional)</label>
                    <input
                      type="text"
                ...
                <div className="pt-2 flex justify-end">
                  <Button disabled={savingWhatsApp} onClick={saveWhatsAppSettings} className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl px-8 font-black uppercase tracking-widest shadow-lg flex items-center gap-2">
                    {savingWhatsApp ? 'Saving...' : <><Save className="w-4 h-4" /> Save WhatsApp Bot Settings</>}
                  </Button>
                </div>
              </div>
            </div>
