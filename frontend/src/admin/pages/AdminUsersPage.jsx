...
const AdminUsersPage = () => {
  ...
  const handleEdit = async () => {
    try {
      setEditSaving(true);
      await adminApi.updateAdminUser(editModal.id, editForm);
  ...
  const handleResetPassword = async () => {
    ...
    try {
      setResetSaving(true);
      await adminApi.resetAdminPassword(resetModal.id, newPassword);
  ...
  const toggleStatus = async (row) => {
    ...
    try {
      await adminApi.updateAdminStatus(row.id, !row.is_active);
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-100">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Add and manage admin accounts for your store.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => { setShowCreate(true); setForm({ full_name: '', email: '', phone: '', password: '', role: 'admin' }); }} className="rounded-xl flex items-center gap-2 px-6 shadow-lg shadow-indigo-100 hover:shadow-indigo-200 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Super Admins</div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Status: Active</div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-slate-400">Disabled Accounts</div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-slate-50/50 border-b border-slate-100">
              <tr>
                <th className="px-8 py-5 text-left text-[11px] font-black text-slate-400 uppercase tracking-wider">Admin Name</th>
                <th className="px-8 py-5 text-left text-[11px] font-black text-slate-400 uppercase tracking-wider">Contact Info</th>
                <th className="px-8 py-5 text-center text-[11px] font-black text-slate-400 uppercase tracking-wider">Role</th>
                ...
                <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-6 text-center">
                    <button 
                      ...
                      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${
                        user.id === me?.id ? 'opacity-50 cursor-not-allowed bg-slate-100 text-slate-400' :
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => { setEditModal(user); setEditForm({ full_name: user.full_name, email: user.email, phone: user.phone || '', role: user.role }); }}
                        className="p-2 rounded-xl border border-slate-100 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all" title="Edit Admin">
                        <Edit3 className="w-4 h-4" />
                      </button>
                      <button onClick={() => { setResetModal(user); setNewPassword(''); }}
                        className="p-2 rounded-xl border border-slate-100 text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-all" title="Reset Password">
                        <Lock className="w-4 h-4" />
                      </button>
                      <button onClick={() => setDeleteTarget(user)}
                        className="p-2 rounded-xl border border-slate-100 text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all" title="Delete Admin">
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md border border-slate-100">
            ...
            <form onSubmit={create} className="space-y-6" autoComplete="off">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500/20 outline-none bg-white"
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button type="submit" disabled={creating}
                  className="flex-1 px-6 py-3 rounded-xl bg-indigo-600 text-white text-sm font-black uppercase tracking-widest hover:bg-indigo-700 shadow-lg shadow-indigo-200 disabled:opacity-50 transition-all">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md border border-slate-100">
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4 pt-4">
                <button onClick={() => setEditModal(null)} className="flex-1 px-6 py-3 rounded-xl border border-slate-200 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
                <button onClick={handleEdit} disabled={editSaving} className="flex-1 px-6 py-3 rounded-xl bg-indigo-600 text-white text-sm font-black uppercase tracking-widest hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all">
